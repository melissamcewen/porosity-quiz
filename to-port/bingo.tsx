...
const createTile = (answer: string): TileItem => ({
  text: answer,
  active: false,
});
      ...
      >
        <hstack alignment="middle start" width={100} grow>
          ...
          <text grow wrap width={100}>
            You are about to share a snapshot of your board as a comment.
            Continue?
          </text>
        </hstack>
        <spacer size="small" />
...
export const App = (context: Devvit.Context): JSX.Element => {
  ...
  const [isConfirmationVisible, setIsConfirmationVisible] =
    context.useState<boolean>(false);
  const [isSuccessVisible, setIsSuccessVisible] =
    context.useState<boolean>(false);
  ...
  const [tiles, setTiles] = context.useState<TileItem[]>(async () => {
    if (currentUserName) {
      const userTiles = await getUserTiles(
        context.redis,
        context.postId!,
        currentUserName,
      );
  ...
  const [themeConfig] = context.useState<ThemeConfig>(async () => {
    const postThemeConfigJSON = await context.redis.get(
      `${context.postId}_theme`,
    );
    ...
    return {
      ...
      appBackgroundColor:
        postThemeConfig.appBackgroundColor || theme.standard.appBackgroundColor,
      ...
      tileBorderActive:
        postThemeConfig.tileBorderActive || theme.standard.tileBorderActive,
      tileText: postThemeConfig.tileText || theme.standard.tileText,
      tileTextActive:
        postThemeConfig.tileTextActive || theme.standard.tileTextActive,
    };
  });
          ...
          <hstack alignment="start middle" height="46px" width="100%">
            <hstack height={100} width={100} alignment="center middle">
              ...
              <hstack
                ...
                backgroundColor={
                  isEasyEyeMode
                    ? staticColors.topRowButtonBgActive
                    : staticColors.topRowButtonBg
                }
              ...
              {currentUserName ? (
                <>
                  ...
                  >
                    <icon
                      name="share"
                      color={staticColors.topRowButtonText}
                      size="small"
                    />
                  </hstack>
                </>
          ...
          <hstack>
            ...
            >
              <hstack alignment="middle center">
                {tiles
                  .slice(0, 4)
                  .map((tile, index) => renderTileRow(index, tile))}
              </hstack>

              <hstack alignment="middle center">
                {tiles
                  .slice(4, 8)
                  .map((tile, index) => renderTileRow(index + 4, tile))}
              </hstack>

              <hstack alignment="middle center">
                {tiles
                  .slice(8, 12)
                  .map((tile, index) => renderTileRow(index + 8, tile))}
              </hstack>

              <hstack alignment="middle center">
                {tiles
                  .slice(12, 16)
                  .map((tile, index) => renderTileRow(index + 12, tile))}
              </hstack>
            </vstack>
            <vstack width="8px" />
          </hstack>
